...
| v0.3 吞吐 | **已完成并定案**：修掉 RX 缓冲缺陷后实测 **4.2~4.7 Mbit/s**；速率由固件内 ARRM 自主决定，主机侧无可改之处（详见「已知问题」） |
...
### 设备收帧上限约 1030 字节（设备侧限制，驱动已尽力）

实测（禁止分片的 `ping`）：payload 905 字节能通、920 字节必丢；有线对照 1472 正常。
用驱动自带的 `rx_debug` 参数抓到 USB 层硬证据：ping 900 时窗口里能看到
`usb_len=1038` 的回复帧；ping 1400（100% 丢包）时同一窗口 181 条记录里 `usb_len`
最大只有 **570**（全是背景流量）⇒ **超限帧被设备直接丢弃且不报错，不是截断**。

与主机无关：RX URB 缓冲从 1024 提到 2048 只把窗口推了 16 字节，所以限制在设备侧。
**影响面有限**：TCP 数据面不受影响，受影响的只有"单包 >905 字节且需要回复"的用法。
...
- **`iw link` 的 tx bitrate 不可当作性能指标**：它只是主机侧速率掩码/记账（报 1.0 Mbit/s），
  实测吞吐是它的 4 倍以上
- **设备收帧上限约 1030 字节**：更大的帧被设备**静默丢弃**（不是截断），
  影响到"单包 >905 字节且需要回复"的 `ping`/UDP；**TCP 数据面不受影响**。
  驱动的 RX 缓冲已扩到 2048 字节，这条限制在设备侧
