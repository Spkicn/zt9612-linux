...
| M3.2 扫描 | 完成，已实机验证（`iw scan` 实测 42 个 BSS） |
| M3.3 关联 / M3.4 数据面 | 未开始 |

加载驱动后可以得到一个 managed 模式的无线接口（名字取决于 systemd 的可预测命名规则，
按 MAC 生成），`iw dev`、`iw scan`、`ethtool -i`、`/dev/zt9612` 都可用。扫描已能列出周围
AP（含 SSID、信道、加密与 HT/VHT 能力）。但关联与数据面尚未实现，因此还无法联网。

> 当前是**被动扫描**（只接收 beacon）：每信道停留 120ms，13 个信道约 1.7 秒。
> 主动扫描需要的 probe request 发送走 TX 数据面，留到 M3.4。
