...
> **v0.3 吞吐已完成**（2026-09-27）。结论：驱动侧已无可改之处 —— 真修复是 RX 缓冲
> （吞吐从"跑不动"→ **4.2~4.7 Mbit/s**），速率由固件内的 ARRM 模块自主决定，
> 主机侧三条通道（描述符 / 速率消息 / 速率掩码）全部无效。
...
- **HT/VHT 能力**：厂商 probe 帧里带 HT Capabilities，说明射频支持；但**预期对吞吐无效**
  （固件自己发 probe/关联、没有主机→固件 HT 能力消息、扫描里的 HT 能力来自固件 ——
  见上文「HT/VHT 也救不了吞吐」）。若要做，只是为了 `iw phy` 显示与真实能力一致，
  且必须注意声明后 mac80211 会开始使用聚合
