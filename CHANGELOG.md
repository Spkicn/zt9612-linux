...
- **设备每帧只能可靠处理约 1 KB**：实测 ping payload 905 字节（IP 包 913）可通、920 字节必丢，
  有线对照 1472 字节正常。驱动已在 `ieee80211_hw.max_mtu` 声明 `ZT_MAX_MTU=900`，
  但**本内核（7.0.0-31）不采纳该字段**（设 900 后仍可把接口 MTU 改到 1500，
  `/sys/class/net/*/max_mtu` 也不存在），所以**接口 MTU 需要用户态设置**——
  默认 MTU=1500 时 TCP 大流量会停滞（`HTTP 200 OK` 之后收不到数据）。
  设置方法：`nmcli connection modify <连接> 802-11-wireless.mtu 900`（或见 README「已知问题」）
- **无线接收缓冲过小（已修）**：RX URB 缓冲原来只有 1024 字节（`MAX_FRAME`），
  装不下满尺寸的 802.11 数据帧，导致 ping payload ≥900 字节的回复被整帧丢弃
  （实测：接口层 RX 计数增量为 0，而空口收到了）。现已把接收路径的缓冲与单帧上限
  提到 2048 字节；修复后实测可通包长从 889 提升到 905 字节
- 运行期 `ip link set mtu` 会让接口短暂失去关联（实测丢 1~2 个 ping，NetworkManager 会重配）；
  另有一次观察到接口消失约 2 分钟后由驱动重新装载并自动重连。两次 dmesg 均无 Oops，
  具体机制未定位，因此改 MTU 建议写进连接配置而不是运行期改
