...
- **主机选速对吞吐没有可观测影响 ⇒ 固件自行选速**（docs/09 §2 的 U1，倾向结论）。
- **但这轮的测量精度不足以做强判定**：同一条件内 5 轮的离散度（2.62~4.75 Mbit/s）
  与环境漂移（第 2 遍整体低约 1.5 Mbit/s）**大于**条件之间的差异，
  因此"强制 1 与强制 11 吞吐相同"这一点只能算**倾向**，不能算证明。
  下一步要么做更严格的测量（固定时间窗、交替采样、更多轮次、同步记录 AP 侧），
  要么转去直接观察固件侧的选速证据（docs/09 §4）。
