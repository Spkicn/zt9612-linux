...
- **无线路径的实际 MTU 只有约 917 字节，而接口声明 1500**（PMTUD 在该网络不生效）：
  MTU=1500 时 TCP/TLS 大包被静默丢弃、TLS 握手超时，**真实上网场景不可用**；
  把 MTU 降到 900 后即可正常下载。这比"吞吐慢"更严重，需在 v0.3 一并处理
- 实测 TCP 吞吐（MTU=900、绑定无线接口）约 3.38 Mbit/s，而 `iw link` 仍报 1.0 Mbit/s
  ⇒ 固件很可能在用更高的实际速率发送，链路速率读数并不代表吞吐上限
- **无线接收缓冲过小（已修）**：RX URB 缓冲原来只有 1024 字节（`MAX_FRAME`），
  装不下满尺寸的 802.11 数据帧，导致 ping payload ≥900 字节的回复被整帧丢弃
  （实测：接口层 RX 计数增量为 0，而空口收到了），表现为 TCP/TLS 大包完全不通。
  现已把接收路径的缓冲与单帧上限提到 2048 字节。修复后实测可通包长
  从 889 提升到 905 字节（IP 包 913）；剩余瓶颈在设备/固件侧，仍在排查
- 改变接口 MTU 后曾观察到无线接口消失、约 2 分钟后由驱动重新装载并自动重连；
  运行期 `ip link set mtu` 还会让接口处于不可用状态（`No route to host`）；
  原因未定位（dmesg 无 Oops），待复现
