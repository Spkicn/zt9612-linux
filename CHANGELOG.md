...
- **收帧上限 ~1024 字节（发送侧无此限制）**：禁止分片的 `ping`/UDP 归因实验
  ...
  * 阈值正好落在 802.11 帧 ≈1024 字节（payload 905 → 帧 981 通过，payload 913 → 帧 989 通过，
  ...
  * 未完成：驱动里为查此事加了 `rx_debug` 参数（打印 USB 层收到的长度，默认关闭），
    但在本机没能取到日志（`rxdbg` 一行未出现，原因待查），因此"设备是截断还是直接丢"
    仍未确证。下一步可用 usbmon（需处理 `timeout` 杀进程导致缓冲不落盘的问题）
    或修好该诊断再取。
