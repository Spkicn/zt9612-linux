...
| 高 | M3.2 扫描的 RX 数据路径格式 | 扫描结果以什么帧从 EP4-IN 上来，这是当前最大的未知 |
| 中 | M3.4 的 TX 描述符布局 | 发送数据帧时主机侧需要填写什么 |
| 中 | 厂商渠道 | 如果能够取得官方 Linux 驱动包（`ZTOP_ACEV100_*_Linux*.tar.gz`），请在 issue 中说明 |
...
它还没有在实机上完成端到端验证，拆分的收益低于风险。等 M3.1 验证通过后，可以按 HIF 层、
消息层、mac80211 层的划分拆分，届时请单独开 issue 讨论。

`driver/_mac_section.c` 是 mac80211 部分的只读摘录，不参与编译。修改 `zt9612.c` 的对应
区段后需要同步更新它。
