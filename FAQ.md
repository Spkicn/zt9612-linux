...
| M3.1 mac80211 注册（`wlan0`） | 代码已写完，尚未实机验证 |
| M3.2 扫描 / M3.3 关联 / M3.4 数据面 | 未开始 |

加载模块后可以得到的是：芯片被点亮、固件运行、可通过 `/dev/zt9612` 收发原始 IPC 帧。
目前还不会有可用的 `wlan0`。
...
答：因为这条路径存在尚未排查清楚的风险。本项目观察到，模块安装到
`/lib/modules/$(uname -r)/extra/` 后，重启时内核会通过 USB modalias 自动加载模块，
随后机器变成「能 ping、能连 22 端口，但 SSH 读不到 banner」的状态，只能硬重启。根因尚未
定位，可能与 xhci 枚举抖动、probe 在 udev worker 中耗时约 7 秒有关。

因此脚本默认写入 `blacklist zt9612`，它只阻止自动加载，手动 `modprobe` 仍然可用。
...
并且固件必须能通过 `linux-firmware` 合法再分发。目前的重点是把 M3 做完。
...
2. M3 的未知项：RX 数据路径格式与 TX 描述符布局
3. 厂商渠道：能够取得官方 Linux 驱动的同学
4. 文档与脚本的修正
