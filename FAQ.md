...
答：目前还不能上网，但驱动已经能完成初始化并注册出无线接口。当前进度如下：

| 里程碑 | 状态 |
|---|---|
| M1 内核态固件装载 | 已实现并在实机验证 |
| M2 IPC 初始化与 `/dev/zt9612` | 已实现并在实机验证（IPC 往返成功） |
| M3.1 mac80211 注册（无线接口） | 已实现并在实机验证（接口按 MAC 命名为 `wlx00b4011a0012`） |
| M3.2 扫描 / M3.3 关联 / M3.4 数据面 | 未开始 |

所以现在能得到的是：芯片被点亮、固件运行、出现一个 managed 模式的无线接口，
并且可以通过 `/dev/zt9612` 收发原始 IPC 帧。扫描与关联尚未实现，因此还连不上 AP。
