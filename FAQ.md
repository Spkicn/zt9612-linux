...
WPA2-PSK 加密、DHCP 与外网 `ping`。实测吞吐 **4.2~4.7 Mbit/s**
...
| v0.3 吞吐 | **已完成并定案**：修掉 RX 缓冲缺陷后实测 4.2~4.7 Mbit/s；速率由固件内 ARRM 自主决定，主机侧无可改之处 |
...
目前的重点是**厂商渠道**（官方 Linux 驱动包或含 RAM 段代码的完整固件）；
吞吐已定案，见 README 路线图。
...
2. **厂商渠道**：能够取得官方 Linux 驱动包、或**包含 RAM 段代码的完整固件**的同学 ——
   这是唯一还能打开"主机影响发射速率"通道的路径（我们手上这份 219 KB 镜像里没有速率控制代码）
3. **RX 描述符里的速率字段**：USB 层抓包没有 radiotap 真值，目前宁缺勿猜
4. **设备收帧上限约 1030 字节**的成因（设备侧是直接丢弃还是有其它规则）
5. 文档与脚本的修正
